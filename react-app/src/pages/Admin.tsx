...
import { CheckCircle, XCircle, Clock, AlertCircle, Search, Mail, Phone, ChevronLeft, ChevronRight, Trash2, Loader2, FileText, X, Plus, Edit, FileSpreadsheet, Globe, BarChart3, TrendingUp, Filter, MousePointer, ExternalLink, Calendar, MapPin, Tag, User as UserIcon, Star, Key, Shield, RefreshCw, Save, Navigation, Link as LinkIcon } from 'lucide-react'
...
export function Admin() {
  ...
  const tabFromUrl = searchParams.get('tab') as 'listings' | 'users' | 'subscriptions' | 'articles' | 'content' | 'analytics' | 'events' | 'mailinglist' | 'headerfooter' | 'pages' | null
  const validTabs = ['listings', 'users', 'subscriptions', 'articles', 'content', 'analytics', 'events', 'mailinglist', 'headerfooter', 'pages']
  const initialTab = (tabFromUrl && validTabs.includes(tabFromUrl)) ? tabFromUrl : 'listings'
  const [activeTab, setActiveTab] = useState<'listings' | 'users' | 'subscriptions' | 'articles' | 'content' | 'analytics' | 'events' | 'mailinglist' | 'headerfooter' | 'pages'>(initialTab)
  
  // Update URL when tab changes
  const handleTabChange = (tab: 'listings' | 'users' | 'subscriptions' | 'articles' | 'content' | 'analytics' | 'events' | 'mailinglist' | 'headerfooter' | 'pages') => {
    setActiveTab(tab)
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-terracotta-50/20 to-gray-100 relative overflow-hidden">
      ...
      <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 relative z-10">
        ...
        <div className="flex gap-2 mb-6 bg-white rounded-xl p-1 shadow-sm border border-gray-200">
          ...
          <button
            onClick={() => handleTabChange('headerfooter')}
            className={`flex-1 px-6 py-3 font-semibold rounded-lg transition-all ${
        ...
        {/* Header/Footer Management Tab */}
        {activeTab === 'headerfooter' && (
          <HeaderFooterManagementSection />
        )}

        {/* Custom Pages Management Tab */}
        {activeTab === 'pages' && (
          <CustomPagesManagementSection />
        )}
...
// Header and Footer Management Component
function HeaderFooterManagementSection() {
  const [footerLinks, setFooterLinks] = useState<any[]>([])
  const [headerSettings, setHeaderSettings] = useState<any[]>([])
  const [footerSettings, setFooterSettings] = useState<any[]>([])
  const [loading, setLoading] = useState(true)
  const [saving, setSaving] = useState(false)
  const [activeSection, setActiveSection] = useState<'header' | 'footer' | 'links'>('header')
  const [editedValues, setEditedValues] = useState<Record<string, string>>({})
  const [editingLink, setEditingLink] = useState<any>(null)
  const [showLinkForm, setShowLinkForm] = useState(false)
  const [newLink, setNewLink] = useState({ section: 'find_support', label: '', url: '', order_index: 0, is_active: true })
  ...
  const fetchData = async () => {
    try {
      ...
      const { data: settings } = await supabase
        .from('site_content_settings')
        .select('*')
        .in('category', ['header', 'footer'])
        .order('category, label')

      if (settings) {
        const header = settings.filter(s => s.category === 'header')
        const footer = settings.filter(s => s.category === 'footer')
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      const updates = Object.entries(editedValues).map(([key, value]) => {
        const setting = [...headerSettings, ...footerSettings].find(s => s.setting_key === key)
        if (!setting || setting.value === value) return null
  ...
  const handleSaveLink = async () => {
    try {
      ...
      setEditingLink(null)
      setNewLink({ section: 'find_support', label: '', url: '', order_index: 0, is_active: true })
      fetchData()
    } catch (error: any) {
  ...
  if (loading) {
    return (
      <div className="text-center py-12">
        <Loader2 className="w-8 h-8 animate-spin mx-auto mb-4 text-terracotta" />
        <p className="text-gray-600">Loading header/footer settings...</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Header & Footer Management</h2>
            <p className="text-gray-600">Edit header navigation, footer content, and manage footer links</p>
          </div>
          <button
        ...
        {activeSection === 'header' && (
          <div className="space-y-4">
            {headerSettings.length === 0 ? (
              <p className="text-gray-500 text-center py-8">No header settings found. Run the SQL script to add default settings.</p>
            ) : (
        ...
        {activeSection === 'footer' && (
          <div className="space-y-4">
            {footerSettings.length === 0 ? (
              <p className="text-gray-500 text-center py-8">No footer settings found. Run the SQL script to add default settings.</p>
            ) : (
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <button
                onClick={() => {
                  setEditingLink(null)
                  setNewLink({ section: 'find_support', label: '', url: '', order_index: footerLinks.length + 1, is_active: true })
                  setShowLinkForm(true)
                }}
                className="px-4 py-2 bg-terracotta text-white rounded-lg font-semibold hover:bg-terracotta-600 transition-colors flex items-center gap-2"
              >
            ...
            {showLinkForm && (
              <div className="border-2 border-terracotta rounded-lg p-4 bg-terracotta-50">
                <div className="flex justify-between items-center mb-4">
                  <h4 className="font-semibold text-gray-900">{editingLink ? 'Edit Link' : 'Add New Link'}</h4>
                  <button onClick={() => { setShowLinkForm(false); setEditingLink(null) }} className="text-gray-500 hover:text-gray-700">
                    <X className="w-5 h-5" />
                  </button>
                </div>
                <div className="space-y-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-4 py-2 border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-terracotta focus:border-terracotta"
                    >
                      <option value="find_support">Find Support</option>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g., Find psychologists in Burnie"
                      className="w-full px-4 py-2 border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-terracotta focus:border-terracotta"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      placeholder="/search?location=Burnie&profession=Psychologist"
                      className="w-full px-4 py-2 border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-terracotta focus:border-terracotta"
                    />
                  </div>
                  <div className="flex gap-2">
            ...
            <div className="space-y-2">
              {['find_support', 'information', 'custom'].map((section) => {
                const sectionLinks = footerLinks.filter(link => link.section === section && link.is_active)
                if (sectionLinks.length === 0) return null

                return (
                  <div key={section} className="border border-gray-200 rounded-lg p-4">
                    <h4 className="font-semibold text-gray-900 mb-3 capitalize">{section.replace('_', ' ')}</h4>
                    <div className="space-y-2">
...
// Custom Pages Management Component
function CustomPagesManagementSection() {
  const [pages, setPages] = useState<any[]>([])
  const [loading, setLoading] = useState(true)
  const [saving, setSaving] = useState(false)
  const [editingPage, setEditingPage] = useState<any>(null)
  const [showPageForm, setShowPageForm] = useState(false)
  const [pageData, setPageData] = useState({ slug: '', title: '', content: '', meta_description: '', meta_keywords: '', is_published: false, order_index: 0 })
  ...
  const handleSavePage = async () => {
    try {
      if (!pageData.slug || !pageData.title || !pageData.content) {
        toast.error('Please fill in all required fields (slug, title, content)')
        return
      }
      ...
      setEditingPage(null)
      setPageData({ slug: '', title: '', content: '', meta_description: '', meta_keywords: '', is_published: false, order_index: 0 })
      fetchPages()
    } catch (error: any) {
  ...
  if (loading) {
    return (
      <div className="text-center py-12">
        <Loader2 className="w-8 h-8 animate-spin mx-auto mb-4 text-terracotta" />
        <p className="text-gray-600">Loading custom pages...</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Custom Pages Management</h2>
          ...
          <button
            ...
            className="px-6 py-3 bg-terracotta text-white rounded-lg font-semibold hover:bg-terracotta-600 transition-colors shadow-md hover:shadow-lg flex items-center gap-2"
          >
            <Plus className="w-5 h-5" />
            Create New Page
          </button>
        ...
        {showPageForm && (
          <div className="border-2 border-terracotta rounded-lg p-6 bg-terracotta-50 mb-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-semibold text-gray-900">{editingPage ? 'Edit Page' : 'Create New Page'}</h3>
              <button onClick={() => { setShowPageForm(false); setEditingPage(null) }} className="text-gray-500 hover:text-gray-700">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    value={pageData.slug}
                    onChange={(e) => setPageData({ ...pageData, slug: e.target.value.toLowerCase().replace(/\s+/g, '-') })}
                    placeholder="about-us"
                    className="w-full px-4 py-2 border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-terracotta focus:border-terracotta"
                  />
                  <p className="text-xs text-gray-500 mt-1">URL: /page/{pageData.slug || 'slug'}</p>
                </div>
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">Order Index</label>
                  <input
                    type="number"
                    value={pageData.order_index}
                    onChange={(e) => setPageData({ ...pageData, order_index: parseInt(e.target.value) || 0 })}
                    className="w-full px-4 py-2 border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-terracotta focus:border-terracotta"
                  />
                </div>
              </div>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={saving}
                  className="px-6 py-3 bg-terracotta text-white rounded-lg font-semibold hover:bg-terracotta-600 transition-colors flex items-center gap-2 disabled:opacity-50"
                >
                  {saving ? (
                    <>
                      <Loader2 className="w-5 h-5 animate-spin" />
        ...
        <div className="space-y-4">
          {pages.length === 0 ? (
            <p className="text-gray-500 text-center py-8">No custom pages yet. Create your first page above.</p>
          ) : (
            pages.map((page) => (
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      {page.is_published ? (
                        <span className="px-2 py-1 bg-green-100 text-green-800 rounded text-xs font-semibold">Published</span>
                      ) : (
                        <span className="px-2 py-1 bg-gray-100 text-gray-800 rounded text-xs font-semibold">Draft</span>
                      )}
                    </div>
                    <p className="text-sm text-gray-600 mb-2">URL: /page/{page.slug}</p>
